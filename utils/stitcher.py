...
class Stitcher2d(object):
    ...
    """
    def __init__(self, depth, patch_size, zoomed_size):
        self.patch_size = patch_size
        self.pad_size = patch_size // 2
        if type(zoomed_size) is not tuple:
            self.zoomed_size = (zoomed_size, zoomed_size)
        else:
            self.zoomed_size = zoomed_size
        self.full_index = ((self.zoomed_size[0] - self.patch_size) / (self.patch_size // 2) + 3) * \
                          ((self.zoomed_size[1] - self.patch_size) / (self.patch_size // 2) + 3)
        self.canvas_size = (depth, self.zoomed_size[0] + self.pad_size * 2, self.zoomed_size[1] + self.pad_size * 2)
    ...
    def stitch(self, patches):
        ...
        for k, v in stitched_img.items():
            unpad = self.pad_size
            stitched_img[k] = v[:, unpad:-unpad, unpad:-unpad]
